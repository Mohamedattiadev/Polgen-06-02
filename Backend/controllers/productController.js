...
^FO30,120^GFA,629,629,17,,::I0C,I0F,001FC,001FE,001FF,003FF8,3F9FF8,3F9FFC,3FBFFC,3JFE,:3JFE004,3JFE03603E003EL0F003FI01,3JFE04C04180410040030C04I081,1JFE10C0408080804002I04I0C1,1IFDE01C0404080404004I04I021,1IFEA03C0408100404004I04I031,0F9FF607C04081004040043C07F0011,0F8FF20D807F01004040040404J09,07C63801004I0804040040404J05,03E11802004I0808040020404J03,01F8CC0C004I0610040030C0400101,007C2018L01E007E00F003F,I0F9,,:::::::::^FS
...
const generateKutuEtiketi = async (products) => {
  ...
  for (const userId of Object.keys(users)) {
      ...
      for (let i = 0; i < userProducts.length; i += 6) {
          ...
          zplData += `^XA
^CI28  ; UTF-8 desteği
^FO30,10^A0N,20,20^FD ${username}^FS
^FO30,40^GB500,3,3^FS
`;
          
          let yOffset = 60;
          pageProducts.forEach(product => {
              zplData += `^FO30,${yOffset}^A0N,16,14^FD${getFormattedDate()}-${product.GroupId || 'N/A'}-${product.index || 'N/A'}       ${product.oligoAdi}^FS\n`;
              yOffset += 20;
          });
